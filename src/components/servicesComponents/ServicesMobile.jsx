...
function ServicesMobile() {
  ...
  return (
    <>
      <section
        ref={container}
        className=" lg:hidden w-screen overflow-x-scroll snap-x snap-mandatory snap-normal scrollbar-hidden   mt-32 "
      >
          ...
          {servicelist.map((service, index) => {
            ...
            return (
              ...
              >
                <h3
                  style={medium.style}
                  className=" ml-6 my-4 text-lg text-mwhite"
                >
                  {title}
                </h3>
      ...
      <div className=" relative left-[80vw] -translate-x-full   ml-9 mt-10  h-7 w-36 overflow-hidden">
        {currServiceTime === 17 && (
          <motion.div
            initial={{ left: "-100%" }}
            animate={{ left: 0 }}
        ...
        >
          {servicelist.map((s, i) => {
            return (
              <p key={i} className="flex justify-between items-center w-36">
                {i != 0 && s.title}
                {i != 0 && (
                  <svg
                    width="15"
