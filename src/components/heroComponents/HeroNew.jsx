...
function HeroNew({loco}) {
  ...
  const slideUp = {
    open: (i) => ({
        y: "0%",
        transition: {duration: 0.5, delay: 0.6+0.02 * i,ease: [0, 0.55, 0.45, 1]}
    }),
    closed: {
        y: "100%",
  ...
  const phrase ="I am a creative web developer with passion in making interactive eye-catching websites that will make your bussiness stand out in the digital world"
  return (
    <section className="myContainer  pt-16">
      <motion.div animate={{opacity:1,transition:{duration:2}}} initial={{opacity:0}} className=" w-full h-[180px] md:h-[250px] lg:h-[300px] xl:h-[400px] relative">
          <video
            src="Mountains (1).mp4"
            loop
            autoPlay
  ...
  {heading.split(" ").map((word, wordIndex) => (
    <span key={wordIndex} className="ml-0 first-of-type:ml-0 relative overflow-hidden inline-flex z-0">
      <motion.span className="mr-3 md:mr-6 first-of-type:ml-0 relative overflow-hidden inline-flex " key={wordIndex}>
      ...
      {wordIndex === 0 ? <span className="w-[690px] h-10 hidden lg:inline-block"></span> : ""}
    </span>
  ))}
</h1>
      </motion.div>
      <div className="flex flex-col lg:flex-row gap-10  justify-between items-center mt-20 md:mt-28 lg:mt-20 xl:mt-28">
        <p className=" text-[16px] text-center lg:text-left md:text-[21px] xl:text-[25px] text-mwhite font-normal scaleCursor max-w-[500px] lg:max-w-[600px] xl:max-w-[800px]">
        {
                    phrase.split(" ").map( (word, index) => {
                        return <span key={index} className={"mr-2 first-of-type:ml-0 relative overflow-hidden inline-flex"}><motion.span variants={slideUp} custom={index} initial={"closed"}  animate={"open"} key={index}> {word}
                      </motion.span></span>
                    })
                }
         </p>
         <div className="h-36 w-full md:w-[300px] relative">
        ...
        <Magnetic>
            ...
            initial={"closed"}
            animate={"open"}
              className="bg-transparent w-44 h-14 md:w-56 md:h-20 lg:size-36 3xl:size-40 border-2 cursor-pointer border-mwhite rounded-full  absolute top-0 right-0  flex justify-center items-center "
            >
              {" "}
        ...
        <Magnetic>
            ...
            initial={"closed"}
            animate={"open"}
              className="bg-transparent w-44 h-14 md:w-56 md:h-20 lg:size-36 3xl:size-40 border-2 cursor-pointer border-mblue rounded-full  absolute top-0 left-0 flex justify-center items-center "
            >
              {" "}
    ...
    <div className=" size-[270px] absolute md:hidden top-1/2 left-0 gradient blur-3xl rounded-full -z-50 -translate-y-1/3 -translate-x-2/3"></div>
    <div className=" size-[270px] md:hidden absolute top-2/3 left-[110%] gradient blur-3xl rounded-full -z-50 -translate-x-1/2"></div>
    </section>
  );
